...
class SharpSpringClient:
    ...
    def get_contact_notes(self, lead_id: str) -> list[dict]:
        ...
        Notes record who booked/visited/sold a lead, deposit amounts and team
        commentary. The getContactNotes method returns a flat list (not a dict)
        keyed by the lead's id parameter.
        """
        all_notes: list[dict] = []
        offset = 0
        while True:
            result = self._call(
                "getContactNotes", {"id": str(lead_id), "limit": 500, "offset": offset}
            )
